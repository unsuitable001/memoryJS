...
export class Pointer {
  ...
  pointedTo(
    MemoryObj /* in case of Local Memory object, pass it. Otherwise, leave it */
  ) {
    if (this.isLocal()) {
  ...
  set point(object) {
    if (this.isLocal()) {
      throw new Error(
        "Pointer.point shorthand is only applicable for global pointers. Use changeValue() function instead"
      );
    } else {
  ...
  get point() {
    if (this.isLocal()) {
      throw new Error(
        "Pointer.point shorthand is only applicable for global pointers. Use pointedTo() function instead"
      );
    } else {
